...
function [image, descriptors, locs, num] = sift(imageFile) %增加了num，每张图像中提取的keypoint个数
...
% if isrgb(image) %如果是rgb彩色图像，则转为灰度图像
%    image = rgb2gray(image);
% end

[rows, cols] = size(image);  %rows * cols 

% Convert into PGM imagefile, readable by "keypoints" executable
f = fopen('tmp.pgm', 'w'); %创建一个临时pgm图像
if f == -1
